      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="角色管理" name="first">
          ...
          <el-pagination
            ...
            :total="$store.state.set.setTotal"
          >
          </el-pagination>
        </el-tab-pane>
  ...
  methods: {
    ...
    del(obj) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        .then(() => {
          this.$store.dispatch('set/dels', obj.id)
        })
        .catch(() => {
          this.$message({
            type: 'info',
    ...
    getSet() {
      console.log(111)
      this.$store.dispatch('set/sets', this.params)
    },
    add() {
      this.$store.dispatch('set/pushAdd', this.ruleForm)
			this.dialogVisible=false
    },
		tree(){
			this.$store.dispatch('set/tree')
		}
  },
  mounted() {},
  watch: {}
